...
int main(int argc, char** argv) {
    ...
    auto ctx = setup_test("Reduce Shared Memory Banks Kernel (on-the-fly A^T + swizzle)",
    ...
    BenchmarkResult result = run_kernel(
        ctx, reduce_shared_memory_banks_kernel<BM, BN, BK, TM, TN>,
        "Reduce Shared Memory Banks (on-the-fly A^T + swizzle)", threads, blocks);
