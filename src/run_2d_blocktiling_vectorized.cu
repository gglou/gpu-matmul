...
int main(int argc, char** argv) {
    ...
    auto ctx = setup_test("2D Block Tiling Vectorized Kernel (on-the-fly A^T + float4)",
    ...
    BenchmarkResult result = run_kernel(
        ctx, blocktiling_2d_vectorized_kernel<BM, BN, BK, TM, TN>,
        "2D Block Tiling Vectorized (on-the-fly A^T + float4)", threads, blocks);
