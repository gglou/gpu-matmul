...
AutotuneResult bench_config(float *d_a_t, float *d_b, float *d_c,
    ...
    const int shmem = (BM * (BK + 1) + BK * BN) * sizeof(float);
...
int main() {
    ...
    std::cout << "Autotuner: reduce_shared_memory_banks (A^T + float4 + swizzle)\n";
    ...
    cublas_init();
    BenchmarkResult cublas_ref = benchmark_cublas(d_a, d_b, d_c_ref, dims, 100);
